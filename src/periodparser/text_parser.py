...
def convert_to_token(period: AbstractPeriod, now: datetime) -> DateTimeToken:
    ...
    if max_fixed == FixPeriod.YEAR:
        ...
        token.date_to = datetime(period.date.year, period.date.month + 1, 1) - timedelta(
            microseconds=1
        )
    ...
    if period.span_direction != 0:
        token.type = (
            DateTimeTokenType.SPAN_FORWARD
            if period.span_direction == 1
            else DateTimeTokenType.SPAN_BACKWARD
        )
...
def take_from_adjacent_i(
    data: DatesRawData, first_index: int, second_index: int, is_linked: bool
) -> Tuple[AbstractPeriod, AbstractPeriod]:
...
def create_date_period(
    match, data: DatesRawData, now: datetime, final_periods: List[DateTimeToken]
) -> bool:
...
def collapse_dates(match, data: DatesRawData, _, is_linked: bool) -> bool:
    ...
    if first_date.min_fixed().value < second_date.min_fixed().value:
        ...
        data.remove_range(
            match.start(2), match.end(2) - match.start(2) + match.end(4) - match.start(4)
        )
    else:
...
def fix_overlap(final_periods: List[DateTimeToken]) -> None:
    ...
    for period in final_periods:
        if period not in skipped:
            overlap_periods = [
                p for p in final_periods if p.overlapping_with(period) and p not in skipped
            ]
...
def do_parse(
    ...
    Recognizer.for_all_matches(
        data.get_pattern, start_periods_pattern, lambda m: collapse_dates(m, data, now, False), True
    )
    Recognizer.for_all_matches(
        data.get_pattern, end_periods_pattern, lambda m: collapse_dates(m, data, now, False), True
    )
    Recognizer.for_all_matches(
        data.get_pattern,
        end_periods_pattern,
        lambda m: take_from_adjacent(m, data, now, False),
        True,
    )
    Recognizer.for_all_matches(
        data.get_pattern,
        start_periods_pattern,
        lambda m: take_from_adjacent(m, data, now, False),
        True,
    )
    ...
    if collapse_distance > 0:
        ...
        Recognizer.for_all_matches(
            data.get_pattern, pattern, lambda m: collapse_closest(m, data, now, False), True
        )
