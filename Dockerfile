...
RUN python -m venv /opt/periodparser-env
ENV PATH /opt/periodparser-env/bin:$PATH
ENV VIRTUAL_ENV /opt/periodparser-env

# Set the working directory.
WORKDIR /workspaces/periodparser/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/periodparser/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/periodparser/ && touch src/periodparser/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/periodparser/
